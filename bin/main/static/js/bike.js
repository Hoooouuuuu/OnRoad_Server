...
window.renderVisibleBikeMarkers = function () {
  ...
  const visibleStations = allBikeStations.filter(station => {
  ...
  // 거리순 정렬 후 최대 100개만 표시
  const center = map.getCenter();
  const top100 = visibleStations
    .map(station => {
      const lat = parseFloat(station.stationLatitude);
      const lng = parseFloat(station.stationLongitude);
      const distance = getDistance(center.lat(), center.lng(), lat, lng);
      return { station, lat, lng, distance };
    })
    .sort((a, b) => a.distance - b.distance)
    .slice(0, 100);

  top100.forEach(({ station, lat, lng }) => {
    const position = new naver.maps.LatLng(lat, lng);
